using Formula81.XrmToolBox.Shared.Parts.Utilities;
using Formula81.XrmToolBox.Tools.AuditGoggles.Models;
using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;

namespace Formula81.XrmToolBox.Tools.AuditGoggles.Mocks
{
    internal class AuditEntityItemsControlItemsSource : IEnumerable<AuditEntity>
    {
        private const string AccountIcon = "";

        private readonly ReadOnlyCollection<AuditEntity> _auditEntityCollection;

        public AuditEntityItemsControlItemsSource()
        {
            var accountIconData = Convert.FromBase64String(AccountIcon);
            _auditEntityCollection = new List<AuditEntity>
            {
                new AuditEntity(1, "account", "Account", true, null, false/*, null, null*/),
                new AuditEntity(10000, "nrq_serviceorder", "Service Order", true, null, true/*, null, null*/),
                new AuditEntity(20000, "msdyn_workorder", "Work Order", true, accountIconData, false/*, null, null*/)
            }.AsReadOnly();
        }

        public IEnumerator<AuditEntity> GetEnumerator()
        {
            return _auditEntityCollection.GetEnumerator();
        }

        IEnumerator IEnumerable.GetEnumerator()
        {
            return GetEnumerator();
        }
    }
}
