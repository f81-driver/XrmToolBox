...
namespace Formula81.XrmToolBox.Shared.Xrm.Resources
{
    public static class XrmResourceRepository
    {
        private const string DefaultEntityIconComponentUrl = "component/Resources/EntityIcons/ico_16_customEntity.gif";
        ...
        {
            var basePackUri = $"pack://application:,,,/{Assembly.GetExecutingAssembly().GetName().Name};";
             var entityIconUrl = objectTypeCode.HasValue
                && EntityIconObjectTypeCodeSet.Contains(objectTypeCode.Value)
                    ? $"component/Resources/EntityIcons/ico_16_{objectTypeCode.Value}.gif"
                        : DefaultEntityIconComponentUrl;
            return new Uri(basePackUri + entityIconUrl);
        }
    }
}
